// src/components/builder/DevicePreview.tsx
'use client';
...
export default function DevicePreview({
  ...
  // ✅ theme ze store (single source of truth)
  const theme = useBuilderStore((s) => s.theme);

  // ✅ drž poslední validní theme (není to default fallback, je to stabilizace proti null-flashi)
  const lastThemeRef = useRef<DesignSystem | null>(null);
  useEffect(() => {
    if (theme) lastThemeRef.current = theme;
  }, [theme]);

  const renderTheme = theme ?? lastThemeRef.current;
  ...
  useEffect(() => {
    const calc = () => {
      ...
      const w = Math.min(
        max,
        Math.max(420, Math.round(window.innerWidth * 0.42))
      );
      setPanelShiftPx(w);
    };
    calc();
  ...
  const setPublishStatusLoaded = useBuilderStore(
    (s) => s.setPublishStatusLoaded
  );
    ...
    async function loadPublishState() {
      ...
      try {
        const res = await fetch(
          `/api/v1/projects/${projectId}/publish-state?pageId=${encodeURIComponent(
            pageId
          )}`,
  ...
  const handleSave = async () => {
    if (!pageId) {
      setSaveState('error');
      alert(
        'Chybí pageId – nejdřív prosím projdi onboarding a spusť generování.'
      );
  ...
  // ✅ Rendruj obsah jen když existuje "renderTheme" (aktuální nebo poslední validní)
  const readyToRenderContent = !generating && hasContent && !!renderTheme;
  ...
  >
    <div className="loader-wrapper">
      {(phase?.trim() ? phase : "Generating").split("").map((ch, i) => (
      ...
      <div className="loader" />
            ...
            {/* ✅ Jen úplně první načtení, kdy theme ještě nikdy nebyl */}
            {!generating && hasContent && !renderTheme && (
              <div
                key="theme-first-loading"
              ...
              >
                ...
                <div className="text-sm text-zinc-600">Načítám vzhled…</div>
              </div>
            )}
            ...
            {readyToRenderContent && (
              <div
                key="content"
                className="h-full w-full overflow-hidden pt-6 px-6 text-zinc-900"
                style={{
                ...
                >
                  ...
                  >
                    {list.map((blk: BlockInstance, index: number) => {
                      ...
                      const modFallback = !def
                        ? getModuleByType(blk.type)
                        : null;
                      const finalDef: any = def ?? modFallback?.definition;
                      const Renderer = finalDef?.Renderer as
                      ...
                      return (
                        <SectionFrame
                          ...
                          editor={editorCfg}
                        >
                          <Renderer block={blk} theme={renderTheme} />
                        </SectionFrame>
                      );
                    })}
                  </div>
