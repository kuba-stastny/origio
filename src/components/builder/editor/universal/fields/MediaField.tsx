...
export type AllowedMedia = readonly MediaType[];
...
export default function MediaField({
  ...
  const v0 = useMemo(() => normalize(value), [value]);
  const allowedSet = useMemo(() => new Set<MediaType>(allowed as MediaType[]), [allowed]);

  // ✅ vynutíme validní typ podle allowed
  const v = useMemo(() => {
    const fallback: MediaType = allowedSet.has("image") ? "image" : "video";
    const type: MediaType = allowedSet.has(v0.type) ? v0.type : fallback;
    return { ...v0, type };
  }, [v0, allowedSet]);

  // když se změnil allowed a typ už není povolený, oprav value
  useEffect(() => {
    if (v.type !== v0.type) {
      onChange({
        type: v.type,
        src: v0.src || "",
        ...(v.type === "image" ? { alt: v0.alt || "" } : {}),
      });
    }
  ...
  const canImage = allowedSet.has("image");
  const canVideo = allowedSet.has("video");
  const showToggle = canImage && canVideo;
  ...
  // ✅ PURE DARK
  const pillWrap =
    "inline-flex w-fit rounded-full bg-zinc-900/60 p-1 shadow-[0_18px_60px_rgba(0,0,0,0.55)]";
  ...
  return (
    <div className="grid gap-2 mt-4">
      ...
      {showToggle && (
        <div className={pillWrap}>
          <button
            ...
            className={[
              pillBtn,
              v.type === "image" ? "bg-zinc-800 text-white" : "text-zinc-300 hover:bg-zinc-950/40",
            ].join(" ")}
          ...
          <button
            ...
            className={[
              pillBtn,
              v.type === "video" ? "bg-zinc-800 text-white" : "text-zinc-300 hover:bg-zinc-950/40",
            ].join(" ")}
      ...
      {/* Inputs */}
      {v.type === "image" ? (
        <div className="grid grid-cols-1 gap-2 md:grid-cols-[2fr_1fr] hidden">
          <input
            type="text"
            className={input}
